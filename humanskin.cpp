...
#define UP_lEG_SKIN_LENGTH 1.6         //大腿的皮肤长度
#define DOWN_lEG_SKIN_LENGTH 1.2       //小腿的皮肤长度

//皮肤的宽高长调节
#define HEADRADIUSZ 0.15
#define HEADRADIUSX 0.12
#define TORSORADIUSZ 0.18    //修改躯干的厚度
#define TORSORADIUSX1 0.35   //修改躯干的宽度
#define TORSORADIUSX2 0.30   //修改躯干的宽度
#define NECKRADIUS 0.05    //修改脖子半径
...
void HumanSkin::makeArm(const std::string& shoulder, const std::string& elbow)
{
  ...
  Limb* upper = new Limb(GL_QUAD_STRIP, Vector3(1.0, 0.0, 0.0));
  Limb* upperCap = new Limb(GL_TRIANGLE_FAN, Vector3(1.0, 0.0, 0.0));
  ...
  double uy1 = 0.005, uy2 = -0.5;
  double ly1 = 0.0, ly2 = -0.5;
  double hy1 = -0.5, hy2 = -0.605;
  ...
  for(i = 0; i <= EDGES; i++)
  {
    ...
    nu->attachments_.push_back(new Vertex::Attachment(shoulder, Vector3(2.0 * x / (ARMRADIUS * ARMRADIUS), 0.0, 2.0 * z / (ARMRADIUS * ARMRADIUS)), 1.0));
    verts_.push_back(nu);

    Vertex* nl = new Vertex();
    nl->attachments_.push_back(new Vertex::Attachment(elbow, Vector3(2.0 * x / (ARMRADIUS * ARMRADIUS), 0.0, 2.0 * z / (ARMRADIUS * ARMRADIUS)), 1.0));
    verts_.push_back(nl);
...
void HumanSkin::makeLeg(const std::string& hip, const std::string& knee)
{
  ...
  // The quad strip vertices
  Limb* upper = new Limb(GL_QUAD_STRIP, Vector3(0.0, 0.0, 0.8));
  Limb* upperCap = new Limb(GL_TRIANGLE_FAN, Vector3(0.0, 0.0, 0.8));
  Limb* lower = new Limb(GL_QUAD_STRIP, Vector3(0.0, 0.0, 0.8));
  Limb* lowerCap = new Limb(GL_TRIANGLE_FAN, Vector3(0.0, 0.0, 0.8));

  // y offsets for the upper arm cylinder, and lower arm cylinder   
  //xmy  修改大小腿长度   
  double uy1 = 0.0, uy2 = -UP_lEG_SKIN_LENGTH;   //uy2 = -0.8
  double ly1 = 0.0, ly2 = -DOWN_lEG_SKIN_LENGTH;  //ly2 = -0.55
...
{
  // The quad vertices
  Limb* foot = new Limb(GL_QUADS, Vector3(139.0/255.0, 69.0/255.0, 19.0/255.0));
  ...
  verts_.push_back(v1);
  Vertex* v2 = new Vertex();
  v2->attachments_.push_back(new Vertex::Attachment(ankle, Vector3(-0.1, 0.05, 0.35), 1.0));
  verts_.push_back(v2);
  Vertex* v3 = new Vertex();
  v3->attachments_.push_back(new Vertex::Attachment(ankle, Vector3(0.1, 0.05, 0.35), 1.0));
  verts_.push_back(v3);
  Vertex* v4 = new Vertex();
  ...
  v6->attachments_.push_back(new Vertex::Attachment(ankle, Vector3(-0.1, -0.05, 0.35), 1.0));
  verts_.push_back(v6);
  Vertex* v7 = new Vertex();
  v7->attachments_.push_back(new Vertex::Attachment(ankle, Vector3(0.1, -0.05, 0.35), 1.0));
  verts_.push_back(v7);
...
void HumanSkin::makeTorso()
{
  ...
  // y offsets for the upper arm cylinder, and lower arm cylinder
  double uy1 = 0.501, uy2 = -0.2;
  double ly1 = -0.2, ly2 = -0.58;
...
void HumanSkin::makeHead()
{
  ...
  // y offsets for the upper arm cylinder, and lower arm cylinder
  //xmy 修改头部参数让脖子显示更长
  double uy1 = 0.451, uy2 = 0.37;
  double ly1 = 0.37, ly2 = 0.05;
